...
async def process_and_store_documents_batch(
    ...
    try:
        ...
        valid_docs = [
            ...
            if doc.get("content") and doc.get("source_url")
        ]
        ...
        batches = [
            valid_docs[i:i + MAX_BATCH_SIZE] 
            for i in range(0, len(valid_docs), MAX_BATCH_SIZE)
        ]
        ...
        async def process_batch(batch: List[Dict[str, Any]], batch_num: int):
            async with semaphore:
                ...
                if len(batches) > 1:
                    print(f"Processing batch {batch_num + 1}/{len(batches)} ({batch_size} documents)...")
