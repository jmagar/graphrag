...
class TestInvalidSignatureRejection:
    ...
    async def test_invalid_signature_rejects_request(
        self, test_client: AsyncClient, sample_webhook_crawl_page: Dict[str, Any]
    ):
...
class TestBackwardsCompatibility:
    ...
    async def test_no_secret_accepts_any_payload(
        self, test_client: AsyncClient, sample_webhook_crawl_completed: Dict[str, Any]
    ):
...
class TestTimingAttackResistance:
    ...
    @patch("app.core.config.settings.FIRECRAWL_WEBHOOK_SECRET", "test-secret-key")
    async def test_uses_constant_time_comparison(self, test_client: AsyncClient):
        """
    ...
    @patch("app.core.config.settings.FIRECRAWL_WEBHOOK_SECRET", "test-secret-key")
    async def test_signature_length_differences(self, test_client: AsyncClient):
...
class TestWebhookSignatureIntegration:
    ...
    async def test_crawl_started_with_signature(self, test_client: AsyncClient):
    ...
    async def test_crawl_failed_with_invalid_signature(
        self, test_client: AsyncClient, sample_webhook_crawl_failed: Dict[str, Any]
    ):
